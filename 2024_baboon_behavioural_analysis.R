...
Baboon_flight_predatorcue_24 <- Baboon_flight_data_24 %>%
  group_by(file_name, predator_cue, Hunting_mode) %>%
  summarise(
    latency_to_flee = first(na.omit(latency_to_flee_s)),  # Get first non-NA value
    .groups = "drop"
  ) %>%
...
ggplot(Baboon_flight_predatorcue_24, aes(x = predator_cue, y = latency_to_flee, fill = Hunting_mode, color = Hunting_mode)) +
  geom_boxplot(alpha = 0.4, outlier.shape = NA) +  # Lighter box plot
  geom_jitter(width = 0.2, size = 1.2, alpha = 0.8) +  # Smaller points
  scale_colour_paletteer_d("nationalparkcolors::Acadia") +  # Color scheme for points
  scale_fill_paletteer_d("nationalparkcolors::Acadia") +  # Color scheme for box fill
...
Baboon_flight_predatorcue_24 <- Baboon_flight_predatorcue_24 %>%
  mutate(log_latency = log(latency_to_flee + 1))

#Strip plot for log latency to flee by predator cue
latency_predcue_plot_24 <- 
  ggplot(Baboon_flight_predatorcue_24, aes(x = predator_cue, y = log_latency, fill = Hunting_mode, color = Hunting_mode)) +
  geom_boxplot(alpha = 0.4, outlier.shape = NA) +  # Lighter box plot
  geom_jitter(width = 0.2, size = 1.2, alpha = 0.8) +  # Smaller points
  scale_colour_paletteer_d("nationalparkcolors::Acadia") +  # Color scheme for points
  scale_fill_paletteer_d("nationalparkcolors::Acadia") +  # Color scheme for box fill
...
flight_frequency_predatorcue_24 <- Baboon_frequency_data_24 %>%
  group_by(predator_cue, file_name, Hunting_mode) %>%  
  summarise(flight_present = max(flight_present), .groups = "drop") %>%  # Check if flight occurred per video
  group_by(predator_cue, flight_present, Hunting_mode) %>%
...
frequency_predcue_plot_24 <-
  ggplot(flight_frequency_predatorcue_24, aes(x = predator_cue, y = Count, fill = Hunting_mode, alpha = flight_present)) +
...
Baboon_vigilance_habitat_24 <- Baboon_vigilance_data_24 %>%
  mutate(Habitat = case_when(
    site %in% c("E02", "F01", "F03", "F07", "I04", "J03", "J13", "N03", "N10", "N11") ~ "Open",
    site %in% c("D05", "D09", "E08", "G06", "G08", "I06", "I08", "I10", "L11") ~ "Closed",
    TRUE ~ NA_character_  # Ensure other values get NA if not listed
  )) %>%
  group_by(Habitat, file_name, site) %>%
...
vigilance_habitat_plot_24 <-
  ggplot(Baboon_vigilance_habitat_24, aes(x = Habitat, y = proportion_vigilant, fill = Habitat, color = Habitat)) +
  geom_boxplot(alpha = 0.4, outlier.shape = NA) +  # Lighter box plot
  geom_jitter(width = 0.2, size = 1.2, alpha = 0.8) +  # Smaller points
  scale_colour_paletteer_d("nationalparkcolors::Acadia") +  # Color scheme for points
  scale_fill_paletteer_d("nationalparkcolors::Acadia") +  # Color scheme for box fill
...
##Make new coloumn  and group by habitat classification by ABR site
Baboon_flight_habitat_24 <- Baboon_flight_data_24 %>%
  mutate(Habitat = case_when(
    site %in% c("E02", "F01", "F03", "F07", "I04", "J03", "J13", "N03", "N10", "N11") ~ "Open",
    site %in% c("D05", "D09", "E08", "G06", "G08", "I06", "I08", "I10", "L11") ~ "Closed",
    TRUE ~ NA_character_  # Ensure other values get NA if not listed
...
ggplot(Baboon_flight_habitat_24, aes(x = Habitat, y = latency_to_flee, fill = Habitat, color = Habitat)) +
  geom_boxplot(alpha = 0.4, outlier.shape = NA) +  # Lighter box plot
  geom_jitter(width = 0.2, size = 1.2, alpha = 0.8) +  # Smaller points
  scale_colour_paletteer_d("nationalparkcolors::Acadia") +  # Color scheme for points
  scale_fill_paletteer_d("nationalparkcolors::Acadia") +  # Color scheme for box fill
...
Baboon_flight_habitat_24 <- Baboon_flight_habitat_24 %>%
  mutate(log_latency = log(latency_to_flee + 1))

#Strip plot for log latency to flee by predator cue
latency_habitat_plot_24 <-
  ggplot(Baboon_flight_habitat_24, aes(x = Habitat, y = log_latency, fill = Habitat, color = Habitat)) +
  geom_boxplot(alpha = 0.4, outlier.shape = NA) +  # Lighter box plot
  geom_jitter(width = 0.2, size = 1.2, alpha = 0.8) +  # Smaller points
  scale_colour_paletteer_d("nationalparkcolors::Acadia") +  # Color scheme for points
  scale_fill_paletteer_d("nationalparkcolors::Acadia") +  # Color scheme for box fill
...
flight_frequency_habitat_24 <- Baboon_frequency_data_24 %>%
  mutate(Habitat = case_when(
    site %in% c("E02", "F01", "F03", "F07", "I04", "J03", "J13", "N03", "N10", "N11") ~ "Open",
    site %in% c("D05", "D09", "E08", "G06", "G08", "I06", "I08", "I10", "L11") ~ "Closed",
    TRUE ~ NA_character_  # Ensure other values get NA if not listed
...
frequency_habitat_plot_24<-
  ggplot(flight_frequency_habitat_24, aes(x = Habitat, y = Count, fill = Habitat, alpha = flight_present)) +
  geom_bar(stat = "identity", position = "fill") +  # Proportional stacking
  scale_fill_paletteer_d("nationalparkcolors::Acadia") +  # Apply Acadia colors to Predator.cue
  scale_alpha_manual(values = c("No Flight" = 0.3, "Flight" = 1)) +  # Adjust transparency
...
Baboon_vigilance_age_sex_24 <- Baboon_vigilance_data_24 %>%
  filter(!is.na(sex)) %>%
  group_by(file_name, sex, age, group_number, offspring) %>%
  summarise(proportion_vigilant = first(na.omit(proportion_vigilant)), .groups = "drop") %>%
  mutate(age_sex_class = case_when(
    sex == "J" & age == "J" ~ "Juvenile",
...
vigilance_prey_plot_24 <-
  ggplot(Baboon_vigilance_age_sex_24, aes(x = age_sex_class, y = proportion_vigilant, fill = age_sex_class, color = age_sex_class)) +
  geom_boxplot(alpha = 0.4, outlier.shape = NA) +  # Lighter box plot
  geom_jitter(width = 0.2, size = 1.2, alpha = 0.8) +  # Smaller points
  scale_colour_paletteer_d("nationalparkcolors::Acadia") +  # Color scheme for points
  scale_fill_paletteer_d("nationalparkcolors::Acadia") +  # Color scheme for box fill
...
#Group by file_name and create a new column for age_sex_class
Baboon_flight_age_sex_24 <- Baboon_flight_data_24 %>%
  filter(!is.na(sex)) %>%
  mutate(age_sex_class = case_when(
    sex == "J" & age == "J" ~ "Juvenile",
    sex == "F" & age == "A" ~ "Female_Adult",
...
ggplot(Baboon_flight_age_sex_24, aes(x = age_sex_class, y = latency_to_flee, fill = age_sex_class, color = age_sex_class)) +
  geom_boxplot(alpha = 0.4, outlier.shape = NA) +  # Lighter box plot
  geom_jitter(width = 0.2, size = 1.2, alpha = 0.8) +  # Smaller points
  scale_colour_paletteer_d("nationalparkcolors::Acadia") +  # Color scheme for points
  scale_fill_paletteer_d("nationalparkcolors::Acadia") +  # Color scheme for box fill
...
#Log transform latency_to_flee because zero skewed
Baboon_flight_age_sex_24 <- Baboon_flight_age_sex_24 %>%
  mutate(log_latency = log(latency_to_flee + 1))

#Strip plot for log latency to flee by predator cue
latency_prey_plot_24 <-
  ggplot(Baboon_flight_age_sex_24, aes(x = age_sex_class, y = log_latency, fill = age_sex_class, color = age_sex_class)) +
  geom_boxplot(alpha = 0.4, outlier.shape = NA) +  # Lighter box plot
  geom_jitter(width = 0.2, size = 1.2, alpha = 0.8) +  # Smaller points
  scale_colour_paletteer_d("nationalparkcolors::Acadia") +  # Color scheme for points
  scale_fill_paletteer_d("nationalparkcolors::Acadia") +  # Color scheme for box fill
...
flight_frequency_age_sex_24 <- Baboon_frequency_data_24 %>%
  filter(!is.na(sex)) %>%
  mutate(age_sex_class = case_when(
    sex == "J" & age == "J" ~ "Juvenile",
    sex == "F" & age == "A" ~ "Female_Adult",
...
#Stacked bar graph for frequency of flight by habitat type
frequency_prey_plot_24 <-
  ggplot(flight_frequency_age_sex_24, aes(x = age_sex_class, y = Count, fill = age_sex_class, alpha = flight_present)) +
  geom_bar(stat = "identity", position = "fill") +  # Proportional stacking
  scale_fill_paletteer_d("nationalparkcolors::Acadia") +  # Apply Acadia colors to Predator.cue
  scale_alpha_manual(values = c("No Flight" = 0.3, "Flight" = 1)) +  # Adjust transparency
